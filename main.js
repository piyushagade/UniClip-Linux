...
// AIzaSyCgzKo7TzJxiltOpD5KH4iP0HU5zp43ev0
// Initialize Firebase
var config = {
...
//set tray icon and options
app.on('ready', function(){
	var path = require('path');
	appIcon = new Tray(path.join(__dirname, 'img/clipboard.png'));
	  ...
	  { label: 'Quit UniClip!',
	    ...
	    click: function() {
			...
			resetReauthorization();
				notify("UniClip!","UniClip is shutting down.", true);
...
function createWindow () {
	...
	mainWindow = new BrowserWindow({
		...
		kiosk: false, 
		title: 'UniClip!', 
		icon : __dirname + '/img/circle_logo.png', 
		movable: true});
...
app.on('ready', function() {
	...
	ipcMain.on('quit', function(event) {
		...
		if (!ret_accept) {
			...
			notify("Cannot register key shortcuts","It looks like some other app is using them.", true);
		}
	});
...
app.on('ready', function() {
  ...
  storage.get('pref', function(error, data) {
    ...
    // If not first run
		...
		notify("UniClip running in background!","Hit Ctrl + Shift + U to show UniClip.", false); 
		...
		try {
			...
			//Set reauth request, Reauthorization required
				...
				fb_reauthorization.on("value", function(snapshot) {
					...
					if(mainWindow == null) createWindow();
					if(mainWindow !== null  && reauth_value === "2" && (mainWindow.isMinimized() || winHidden)){
						mainWindow.restore();
						mainWindow.show();
						winHidden=false;
					}
...
function onVerified() {
	...
	fb_data.on("value", function(snapshot) {
	    ...
	    if(automatic_sync_enabled){
	       notification_body = 'Clipboard updated.';
	       clipboard.writeText(decrypt(decrypt(decrypt(r_data))));           
	    } 
...
ipcMain.on('logout', function(event) {
	...
	notify("UniClip!","This desktop has been successfully unregistered.", true);
